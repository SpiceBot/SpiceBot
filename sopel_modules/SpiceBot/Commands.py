...
def query_detection_nick(bot, trigger):
    ...
    if not letters_in_string(trigger.sb['com']):
        return

    commands_list = dict()
    for com in commands.dict['commands']['nickname'].keys():
        if com not in commands_list.keys():
            commands_list[com] = commands.dict['commands']['nickname'][com]
    ...
    if trigger.sb['com'].endswith("+"):
        ...
        if trigger.sb['com'].lower() not in list(commands_list.keys()):
            ...
            closestmatches = similar_list(trigger.sb['com'], list(commands_list.keys()), 10, 'reverse')
...
def query_detection(bot, trigger):
    ...
    commands_list = dict()
    for commandstype in commands.dict['commands'].keys():
        if commandstype not in ['rule', 'nickname']:
            for com in commands.dict['commands'][commandstype].keys():
                if com not in commands_list.keys():
                    commands_list[com] = commands.dict['commands'][commandstype][com]
